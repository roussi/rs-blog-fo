...
90% of nowadays projects use RESTful API to expose there services, with spring boot this become a piece of cake. But what about Validation (the main topic for this post), Exceptions and Error handling ( will be in Part 2 ).\nWhen dealing with those kind of services ? what can we do when something goes wrong ? and how spring can help us ? \nThis post will be one of many posts related to Spring framework that will be available in my personal blog
( coming soon ! ). This post not only cover input validation check, but also how to return a proper response back to the client when something goes wrong, so lets start ( like Linus Torvalds said "Talk is cheap, show me the code" ).
 \nValidation handling is one of the important key that gives your API a good reputation, knowing that whenever a client send you a request you will control it to check if it has errors, bad format, missing required values, some business to be there ...\n
Spring boot out of the box validation Spring provide us some simple out of the box validation, that is checking that some required fields are present in the request via the required property (that is set to true by default) of @PathVariable & @RequestParam annotations :
